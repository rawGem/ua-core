...
var ensureSignedIn = function() {
  if (!Meteor.userId()) {
      Tracker.nonreactive(function () {
        AccountsTemplates.setPrevPath(FlowRouter.current().url);
      });
      ...
      var defaultLayout = AccountsTemplates.options.defaultLayout || FlowRouter.options.layoutTemplate;
...
//Iron.FlowRouter.plugins.ensureSignedIn = function (router, options) {
...
AT.prototype._init = function() {
    ...
    //    Tracker.nonreactive(function () {
    //        var currentPath = FlowRouter.current().url;
...
AT.prototype.linkClick = function(route){
    ...
    if (path === "#" || AccountsTemplates.avoidRedirect || path === FlowRouter.current().route.path())
        AccountsTemplates.setState(route);
    else
        Meteor.defer(function(){
            FlowRouter.go(AccountsTemplates.getRouteName(route));
        });
};
